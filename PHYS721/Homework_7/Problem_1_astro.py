...
#astropy fitting
bck_init = models.Polynomial1D(degree=2)
fit_bck = fitting.LinearLSQFitter()
b = fit_bck(bck_init, xdata, ydata)

g_init = models.Gaussian1D(amplitude=1E10, mean=mass, stddev=width) + models.Polynomial1D(degree=2)
...
plt.plot(xdata, g(xdata),'b--', lw=4, 
    label=r'$\mathrm{Poly\ bkg\ gaus\ peak\ :\ Mass=%.4f \ GeV,}\ \Gamma=%.4f $' %(g.parameters[1],g.parameters[2]))
plt.plot(xdata, g(xdata),'b--', lw=4, 
    label=r'$\mathrm{Poly\ bkg\ gaus\ peak\ : \ \chi^{2} = %.4f \ \ \ \sigma = %.1f}$' %(c2,0))

plt.plot(xdata, b(xdata), 'k-', label='$\mathrm{Background}$', lw=3)
plt.plot(xdata, g(xdata)-b(xdata), 'r-', label='$\mathrm{Gausian}$', lw=2)

signal = []
for i in xrange(num_bins):
    temp = ydata[i] - g(xdata[i])
    if temp <= 0:
        temp = 0
    signal.append(temp)
