...
void problem_1(){
	...
	TH1D *energy_hist = new TH1D("Energy","Energy",40,100,160);
	//declare the canvas to display hist
	TCanvas *c1 = new TCanvas("c1","c1",10,10,900,900);
	TF1 *fitFcn = new TF1("fitFcn",fitFunction,90,180,8);
	...
	fitFcn->SetParameter(6,1.0);
	fitFcn->SetParameter(8,1.66); // width
	fitFcn->SetParameter(7,126.5); // peak
	energy_hist->Fit("fitFcn", "V+");

	// improve the picture:
	TF1 *backFcn = new TF1("backFcn",background,100,160,3);
	backFcn->SetLineColor(1);
	backFcn->SetLineStyle(1);
	TF1 *signalFcn = new TF1("signalFcn",lorentzianPeak,100,160,3);
