...
def BW_NonR(Energy,Mass,Gamma,z):
    return (((Gamma/(2.0*np.pi)))/((Energy-Mass)**2.0 + (Gamma/2.0)**2.0)) + z
...
popt_2, pcov_2 = curve_fit(BW_NonR, xdata, ydata, p0=np.array([1.02,0.0043,0]))
perr_2 = np.sqrt(np.diag(pcov_2))
plt.plot(xdata,BW_NonR(xdata,popt_2[0],popt_2[1],popt_2[2]),'r--', lw=4,
    label=r'$\mathrm{Non-Rel. \ BW:\ Mass=%.6f \pm %.6f \ GeV,}\ \Gamma=%.6f \pm %.6f$' %(popt_2[0], perr_2[0], popt_2[1], perr_2[1]))
print "BW_non = ",sum(((BW_NonR(xdata,popt_2[0],popt_2[1],popt_2[2])-ydata))**2)
print popt_2[2]
